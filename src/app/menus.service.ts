...
export class MenusService {
  ...
  getMenus() {
    return this.http.get('https://localhost:44309/api/menus');
  }
  saveMenu(menutitle: string, menuactive: string,branchid) {
    this.http.post<any>( 'https://localhost:44309/api/menus/', { Menu_Title: menutitle, Menu_Active: menuactive , BranchId : branchid}).subscribe(data => { this.getMenus(); });
  }
